...
const SingleItem = ({ item }: { item: any }) => {
  ...
  const handleIncreaseQuantity = () => {
    useStore.setState((state: any) => ({
      cart: state.cart.map((crt: any) =>
        crt.id === item.id
          ? { ...crt, quantity: (crt.quantity ?? 1) + 1 }
          : crt
      )
    }));
  };
  ...
  const handleDecreaseQuantity = () => {
    useStore.setState((state: any) => ({
      cart: state.cart.map((crt: any) => {
        if (crt.id === item.id && crt.quantity > 1) {
          return { ...crt, quantity: crt.quantity - 1 };
        }
        return crt;
      })
    }));
  ...
  return (
    <div className="flex items-center border-t border-gray-3 py-5 px-7.5">
      ...
      <div className="min-w-[180px]">
        <p className="text-dark">${item?.salesPrice}</p>
      </div>
      ...
      <div className="min-w-[200px]">
        <p className="text-dark">${item?.salesPrice * item?.quantity}</p>
      </div>

...
import Link from "next/link"; // Added for product card links

// --- STATIC FALLBACK DATA (For when backend is offline) ---
const MOCK_STATIC_PRODUCT = {
  ...
  sku: "HDPH-500-BLK",
  // Using the specific filenames you provided
  images: [
...
const RELATED_PRODUCTS = [
  {
    id: 1,
    title: "BassPro Wireless Earbuds",
    category: "Audio",
    price: 129.00,
    oldPrice: 159.00,
    rating: 4.5,
    image: "/images/products/product-2-sm-1.png" // Reusing your existing images for demo
  },
  {
    id: 2,
    ...
    oldPrice: null,
    rating: 4.8,
    image: "/images/products/product-2-sm-2.png"
  },
  {
    id: 3,
  ...
  },
  {
    ...
    oldPrice: 59.00,
    rating: 4.0,
    image: "/images/products/product-7-b-2.png"
  }
];
...
// --- MAIN COMPONENT ---
const ProductDetailsContent = ({ product: initialProduct }: { product: any }) => {
  
  // --- DATA NORMALIZATION ---
  // Use MOCK_STATIC_PRODUCT if initialProduct is null/undefined (Backend Offline Mode)
  const baseProduct = initialProduct || MOCK_STATIC_PRODUCT;

  const product = {
    ...baseProduct,
    images: baseProduct?.images?.length ? baseProduct.images : [{ file_url: "/placeholder.png" }],
    colors: baseProduct?.colors || [],
    sizes: baseProduct?.sizes || [],
    reviews: baseProduct?.reviews || [],
    customSpecifications: baseProduct?.customSpecifications || [],
    tags: baseProduct?.tags || [],
    seller: baseProduct?.seller || {
      name: baseProduct?.Shop?.name || "Verified Shop",
      rating: 4.5,
      shipsOnTime: "95%",
      chatResponse: "90%"
    }
  ...
  // Safe initialization of selectors
  ...
  return (
    <>
    <Breadcrumb title={product.title} pages={['product', product.slug || 'details']}/>
    <section className="bg-white py-10 lg:py-20 font-sans text-[#1F2937]">
      <div className="max-w-[1170px] mx-auto px-4 sm:px-8">
            ...
            <div className="relative overflow-hidden rounded-xl border border-[#E5E7EB] bg-[#F9FAFB] min-h-[400px] lg:h-[500px] flex items-center justify-center group">
              ...
              {product.regularPrice > product.discountedPrice && (
                <span className="absolute top-5 left-5 bg-[#EF4444] text-white text-xs font-bold px-3 py-1.5 rounded uppercase tracking-wider">
                    Sale -{Math.round(((product.regularPrice - product.discountedPrice) / product.regularPrice) * 100)}%
                </span>
              )}
            </div>
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="flex items-center gap-2">
                <StarRating rating={4.5} /> {/* Replace with calculated average rating if available */}
              ...
              <span className={`text-sm font-semibold px-2 py-0.5 rounded ${product.stock > 0 ? 'text-[#16A34A] bg-[#F0FDF4]' : 'text-[#EF4444] bg-[#FEF2F2]'}`}>
            ...
            <div className="flex items-end gap-3 mb-6">
              <span className="text-3xl font-bold text-[#2563EB]">${product.discountedPrice || product.regularPrice}</span>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <button
                 disabled={isInCart || quantity === 0 || product.stock === 0}
                 onClick={() => addToCart({
                   ...product, 
                   quantity: quantity,
                   selectedColor,
            ...
            <div className="mt-6 flex flex-wrap gap-6 text-sm text-[#6B7280]">
               ...
               <span className="flex items-center gap-1">
                 ...
                 {product.cashOnDelivery ? "Cash on Delivery" : "Pre-payment Only"}
               </span>
            </div>
        ...
        {/* --- TABS SECTION --- */}
        <div className="bg-[#F9FAFB] rounded-2xl p-6 lg:p-10 border border-[#F3F4F6] mb-20">
          ...
          <div className="min-h-[200px]">
            ...
            {activeTab === "description" && (
              <div className="animate-fade-in-up">
                ...
                {/* Optional: Generic highlights if not in DB, or remove this block */}
                <div className="grid md:grid-cols-2 gap-6 mt-8">
                   <div className="bg-white p-5 rounded-lg border border-[#F3F4F6] shadow-sm">
                      <h4 className="font-bold text-[#111827] mb-2">Premium Quality</h4>
                      <p className="text-sm text-[#6B7280]">Manufactured with high precision and durable materials.</p>
                   </div>
                   <div className="bg-white p-5 rounded-lg border border-[#F3F4F6] shadow-sm">
                      <h4 className="font-bold text-[#111827] mb-2">Customer Satisfaction</h4>
                      <p className="text-sm text-[#6B7280]">Rated highly by users for performance and reliability.</p>
                   </div>
                </div>
              </div>
            )}
            ...
            {activeTab === "specs" && (
              <div className="animate-fade-in-up max-w-3xl">
                <table className="w-full text-left border-collapse bg-white rounded-lg overflow-hidden shadow-sm border border-[#E5E7EB]">
                  <tbody>
                    {product.customSpecifications.map((spec: any, idx: number) => (
            ...
            {activeTab === "reviews" && (
              <div className="grid lg:grid-cols-2 gap-12 animate-fade-in-up">
                ...
                <div className="space-y-6">
                  <h3 className="text-xl font-bold text-[#111827]">{product.reviews.length} Reviews for {product.title}</h3>
                  {product.reviews.length > 0 ? (
                      product.reviews.map((rev: any) => (
                        <div key={rev.id || Math.random()} className="bg-white p-6 rounded-xl border border-[#F3F4F6] shadow-sm">
                          <div className="flex justify-between items-start mb-3">
                            <div className="flex items-center gap-3">
                               <div className="w-10 h-10 rounded-full bg-[#E5E7EB] flex items-center justify-center text-[#6B7280] font-bold">
                                 {rev.user ? rev.user.charAt(0) : 'U'}
                               </div>
                               <div>
        ...
        <div className="mt-12 lg:mt-24 border-t border-[#E5E7EB] pt-12">
            <h2 className="text-2xl font-bold text-[#111827] mb-8">You May Also Like</h2>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {RELATED_PRODUCTS.map((item) => (
                    <div key={item.id} className="group bg-white rounded-xl border border-[#E5E7EB] overflow-hidden hover:shadow-lg transition-all hover:border-[#BFDBFE]">
                        {/* Image */}
                        <div className="h-48 bg-[#F9FAFB] relative overflow-hidden flex items-center justify-center">
                            <img 
                                src={item.image} 
                                alt={item.title} 
                                className="w-full h-full object-contain p-4 transition-transform duration-500 group-hover:scale-105"
                            />
                            {item.oldPrice && (
                                <span className="absolute top-2 left-2 bg-[#EF4444] text-white text-[10px] font-bold px-2 py-0.5 rounded uppercase">
                                    Sale
                                </span>
                            )}
                        </div>
                        ...
                        {/* Content */}
                        <div className="p-4">
                            <span className="text-xs text-[#6B7280] uppercase tracking-wide font-medium">{item.category}</span>
                            <h3 className="font-bold text-[#111827] text-sm mt-1 mb-2 line-clamp-2 min-h-[40px]">
                                {item.title}
                            </h3>
                            
                            <div className="flex items-center gap-2 mb-3">
                                <StarRating rating={item.rating} />
                                <span className="text-xs text-[#9CA3AF]">({item.rating})</span>
                            </div>

                            <div className="flex items-center justify-between">
                                <div className="flex flex-col">
                                    <span className="font-bold text-[#2563EB] text-lg">${item.price}</span>
                                    {item.oldPrice && (
                                        <span className="text-xs text-[#9CA3AF] line-through">${item.oldPrice}</span>
                                    )}
                                </div>
                                <button className="w-8 h-8 rounded-full bg-[#F3F4F6] flex items-center justify-center text-[#111827] hover:bg-[#111827] hover:text-white transition-colors">

...
const ProductItem = ({ item, isEvent = false }: ProductItemProps) => {
  const { openModal } = useModalContext();
  const { addToWishList, removeFromWishList, wishlist, addToCart, cart } = useStore()
  const isWishListed = wishlist.some(list => list?.id === item?.id)
  const isInCart = cart.some((crt:any) => crt?.id === item?.id)
  const { user } = useUser()
  ...
  useEffect(() => {
    if (!isEvent || !item?.endingDate) return;

    const end = new Date(item?.endingDate).getTime();
  ...
  }, [isEvent, item?.endingDate]);
  //   const [timeLeft, setTimeLeft] = useState("");

  // useEffect(() => {
  //   if (!isEvent) return;

  //   const end = new Date("2025-12-12T16:25:12.345Z").getTime();
  ...
  //     setTimeLeft(`${days}d ${hours}h ${minutes}m ${seconds}s left with this price`);
  //   }, 100); // updates every minute
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-[#F6F7FB] min-h-[270px] mb-4">
        {/* <Image src={images||""} alt="" width={250} height={250} /> */}
        <img
          src={images || ""}
          alt=""
          width={250}
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          ...
          onClick={()=>!isInCart&&addToCart({...item,quantity:1},user,location,deviceInfo)}
          >
            Add to cart
          </button>
          ...
          >
            ...
            <Heart size={16} fill={isWishListed ? "red" : 'transparent'} stroke={isWishListed?"red":"#4B5563"}
              className="hover:scale-110 transition"
              onClick={() => {
                isWishListed ?
                  removeFromWishList(item.id,user, location, deviceInfo) :
                  addToWishList({ ...item, quantity: 1 },user,location,deviceInfo)
              }}
            />
          </button>
        </div>
      ...
      {/* Timer line under image, no style changes elsewhere */}
      {isEvent && item?.endingDate && (
        <div className="mb-2 text-xs font-medium text-[#dc2626]">
          Ends in:{" "}
          <span>
            {timeLeft}
          </span>
      ...
      <span className="flex items-center gap-2 font-medium text-lg">
        <span className="text-dark">
          ${item.discountedPrice || item.salesPrice}
        </span>

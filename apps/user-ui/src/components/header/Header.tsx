...
const Header: React.FC<HeaderProps> = ({
  ...
  const removeHeader = ["signin", "signup"]

  // Check if current path contains any removeHeader route
  const shouldHideHeader = removeHeader.some(route =>
  ...
  return (
    <header
      className={`fixed left-0 top-0 w-full z-9999 bg-white transition-all ease-in-out duration-300 ${stickyMenu ? 'shadow-2 ' : ''
        }`}
    >
      <div className="max-w-[1170px] mx-auto px-4 sm:px-7.5 xl:px-0">
        ...
        >
          ...
          {/* Header Right - Support, Account, Cart */}
          <div className="flex w-full lg:w-auto items-center gap-7.5">
            ...
            <div className="flex w-full lg:w-auto justify-between items-center gap-5">
              <div className="flex items-center gap-5">
                {/* Sign In */}
                <Link href={isLogged ? "/profile" : "/signin"} className="flex items-center gap-2.5">
                  {
                    isLogged ?
      ...
      {/* Navigation Bar */}
      <div className={`border-t border-gray-3 ${stickyMenu?'hidden':''}`}>
        <div className="max-w-[1170px] mx-auto px-4 sm:px-7.5 xl:px-0">
...
>
  <span className="relative inline-block">
    ...
    {/* Wishlist Badge */}
      ...
      <span className="flex items-center justify-center font-medium text-2xs absolute -right-2.5 -top-2.5 bg-blue w-4.5 h-4.5 rounded-full text-white">
        {wishlist.length}
      </span>
  </span>
